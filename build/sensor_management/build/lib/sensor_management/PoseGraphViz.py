...
class PoseGraphViz(Node):
    ...
    def pose_graph_callback(self, msg: PoseGraph):
        ...
        for i, node in enumerate(msg.nodes):
            marker = self.create_arrow_marker(i, node.x, node.y, node.theta, 0.3, color=self.color_rgba(0.0, 1.0, 0.0, 1.0))
        ...
        for i, constraint in enumerate(msg.pose_constraints):
            marker = self.create_line_marker(i + 1000, msg.nodes[constraint.pose1_id], msg.nodes[constraint.pose2_id], color=self.color_rgba(0.0, 0.0, 1.0, 1.0))
            marker_array.markers.append(marker)
        ...
        for i, constraint in enumerate(msg.landmark_constraints):
            if constraint.pose_id < len(msg.nodes):
                ...
                marker = self.create_line_marker(i + 2000, pose, Point(x=landmark_x, y=landmark_y, z=0.0), color=self.color_rgba(1.0, 0.0, 0.0, 1.0))
